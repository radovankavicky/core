...
\chapter{Bootstrap}
\SweaveOpts{prefix.string=tmp/asymptotics/bootstrap}
\SweaveOpts{stylepath=TRUE}
\SweaveOpts{keep.source=TRUE}
<<echo=FALSE>>=
options(width=60, continue = " ", warn = 1)
@
\DefineVerbatimEnvironment{Sinput}{Verbatim}{fontfamily=TeXGyrePagella}
\DefineVerbatimEnvironment{Soutput}{Verbatim}{fontfamily=TeXGyrePagella}
\DefineVerbatimEnvironment{Scode}{Verbatim}{fontfamily=TeXGyrePagella}
...
First, define a simple function that will generate \texttt{nsims}
averages, each of \texttt{nobs} draws from the distribution specified
in \texttt{rerror}, as follows
<<>>=
rstats <- function(rerror, nobs, stat = mean,
                   nsims = 150000) {
...
<<fig=TRUE,width=2,height=7,echo=false>>=
par(mfrow = c(3,1))
nicehist(runif, 2)
nicehist(runif, 5)
nicehist(runif, 50)
...
\item Of course, another approach would be to use the \emph{best}
  ...
  Suppose we observe the data $y₁,...,y_n$ (univariate for now), and
  let $Y^*$ be a random variable s.t.
  \begin{equation*}
    Y^* =
    \begin{cases}
      y₁ & \text{with probability } 1/n \\
      y₂ & \text{with probability } 1/n \\
      ⋮ \\
...
We can mimic a testing scenario.  The endpoints for 5\%
one-sided intervals based on the bootstrap distribution would be
<<>>=
quantile(boots, c(0.05, 0.95))
@
and would have true coverage of
<<>>=
mean(trues >= quantile(boots, 0.05))
mean(trues <= quantile(boots, 0.95))
@ 
...
\item One potential use of the bootstrap is to estimate the variance
  ...
  \item Draw a random sample of size $n$ from $\Fh$ with replacement
    and estimate the statistic on that sample; call it $\θh^*_b$.
  \end{itemize}
  This gives $\θh^*_1,...,\θh^*_B$ replications of the estimator.
  Calculate their variance, and this is an estimator of the variance
  of $\θh$.

  We can illustrate this for the median
<<>>=
boots <- replicate(1000, median(sample(Y, replace = TRUE)))
var(boots)
@
...
<<>>=
boots <- replicate(1000, median(sample(Y, replace = TRUE))^(1.2))
var(boots)
@
which is a trivial code change.  Now, the bootstrap doesn't always
work (you need conditions like you need for delta method to work).
...
\item A complication shows up when you want to use the bootstrap for
  ...
  resample from $Y₁ - m(Y),...,Y_n - m(Y)$.

<<>>=
m <- median(Y)
boots <- replicate(1000, median(sample(Y - m, replace = TRUE)))
@

Here the median is \Sexpr{m} and the critical value for a 95\%
two-sided test is
<<>>=
quantile(abs(boots), 0.95)
@
