...
\item Although we have presented everything so far for random
  variables, nothing needs to be restricted to a single dimesion.  We
  can define the \emph{joint density} and \emph{joint distribution} of
  a random $k$-variable $X$ just as before.  Now $f(x₁,…,x_k)$
  describes the relative frequency that $X$ will take on particular
  values in $\RRᵏ$, and we can define the joint CDF as
  \begin{equation*}
    F(x₁,…,x_k) = \Pr[X₁ ≤ x₁,…, X_k ≤ x_k].
  \end{equation*}
  The density and distribution functions for individual elements of
  $X$ are called their \emph{marginal densities} and \emph{martinal
  distributions}.

\item The marginal distribution and density functions can be derived
  ...
  \end{equation*}
  This can be motivated directly by working with Dirac's δ-function,
  but can also seen by starting with the marginal CDF and then
...
\item\begin{defn} Let $\Fs$ be a sigma-field on a sample space $S$.  An
  $\Fs$-measurable function $X$ from $S$ to $\RR$ is a \emph{random
  variable}.
  \end{defn}

\item Remember the definition of \emph{$\Fs$-measurable}: $X$ is an
...
\item From this approach, the original probability space is the
...
\item This idea can be extended analogously to densities and
  distribution functions.\sidenote{For a more rigorous extension, we
  will need to wait for expectations.}  For random variables $X$ and
  $Y$, the density of $X$ given a hypothetical value for $Y$ can be
  defined as
  \begin{equation*}
    f_X(x ∣ Y) = f(x, Y) / f_Y(Y).
  \end{equation*}
  For a fixed value of $x$, note that the conditional density is a
  random variable that varies with $y$.

  This definition can have the same interpretation as for events:
  $f(x, y) = f_X(x ∣ y) f_Y(y)$.
...
\item A similar definition holds for information sets:
  \begin{defn}
...
\item It is then straightforward to verify that, for independent
  ...
  \begin{align*}
    ...
    &= \Pr[g(X₁) ∈ I₁] \Pr[h(X₂) ∈ I₂]
  \end{align*}
  where $g^{-1}(I₁) = \{x : g(x) ∈ I₁\}$, we don't need $g$ to be
  invertible.
