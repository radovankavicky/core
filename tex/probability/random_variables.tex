...
\item If we want to let $X$ take on the entire range of values in \RR\
  ...
  For any interval $(a, b]$,\sidenote{Remember, $(a,b] = \{x ∈ \RR ∣ a
    < x ≤ b\}$.  We're using intervals that are open on the left and
    closed on the right because they form a disjoint partition of the
    real line.  Unlike open intervals, we have $(a,b] ∪ (b,c] = (a,c]$
    (note that $(a,b) ∪ (b,c) ≠ (a,c)$) and unlike closed intervals,
    $(a,b] ∩ (b,c] = ∅$ (whereas $[a,b] ∩ [b,c] = \{b\}$).} we can
  define
  \begin{equation*}
    \Pr[ X ∈ (a,b] ] = \lim_{c ↓ a} ∫_c^b f(x) dx,
  \end{equation*}
  and for any set $A$ that can be written as the union of a countable
  number of disjoint intervals, $A = ⋃_i (a_i, b_i]$, we can define
  \begin{equation*}
    \Pr[ X ∈ A ] = \sum_i \Pr[ X ∈ (a_i, b_i] ]
  \end{equation*}
