% The files `AUTHORS_standalone.tex`, `LICENSE_standalone.tex`, and
% `bibliography_standalone.tex` are available if you want to
% distribute the author list, FDL, and bibliography on their own.
\addtocontents{toc}{\protect\setcounter{tocdepth}{0}}

\newpage
\part*{Complete list of authors}%
\addcontentsline{toc}{part}{Appendix A: Complete list of authors}
\input{AUTHORS}
\newpage
\part*{GNU Free Documentation License}%
\addcontentsline{toc}{part}{Appendix B: GNU Free Documentation License}
\input{LICENSE}
\newpage
\part*{References}%
\addcontentsline{toc}{part}{References}
\bibliography{common/references,CITATION}